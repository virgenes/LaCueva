  ...
  const dispatchAction = useCallback((action: CombatAction) => {
    ...
    setCombatState(prev => {
      if (action.type === 'flee') {
        ...
        if (next.phase === 'victory') { setTimeout(() => onFlee(), 0); return prev; }
        return next;
      }
      const next = CombatEngine.act(prev, action, isSpanish);
      ...
      <div className="bg-black border-t-0">
        <div className="h-10 border-b-2 border-white/20 px-3 flex items-center overflow-hidden">
          <AnimatePresence mode="wait">
            {latestLog ? (
              <motion.p key={latestLog.timestamp} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-pixel text-[7px] text-white/50">{char.stats.hp}/{char.stats.maxHp}</span>
                </div>
              </div>
            );
          })}
          ...
          <div className="p-2">
            ...
            {targetMode && (
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => { sfxSelect(); setTargetMode(false); setSelectedSkill(null); setActiveMenu('main'); }}
        ...
        {!isPlayerTurn && combatState.phase === 'enemy_select' && (
          <div className="p-3 flex items-center justify-center">
            <motion.p animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 1.5, repeat: Infinity }}
              ...
              {isSpanish ? 'Turno del enemigo...' : 'Enemy turn...'}
            </motion.p>
          </div>
        )}
      </div>
