...
export const GameMenu: React.FC<GameMenuProps> = ({
  gameState, gameData, party, onClose, onUseItem,
}) => {
      ...
      <div className="w-28 sm:w-36 border-r-2 border-white/30 flex flex-col">
          ...
          {selectedChar && (
            <>
              ...
              <div className="mt-1 space-y-0.5">
                <p className="font-pixel text-[7px] text-white/70">LV: {selectedChar.stats.level}</p>
                <p className="font-pixel text-[7px] text-white/70">HP: {selectedChar.stats.hp}/{selectedChar.stats.maxHp}</p>
                <p className="font-pixel text-[7px] text-yellow-400">G: {gameState.gold || 0}</p>
              </div>
            </>
          )}
        </div>
          ...
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`w-full px-2 py-1.5 font-pixel text-[9px] text-left border-2 rounded-sm transition-colors
                ${activeTab === tab.id ? 'border-white text-white bg-white/10' : 'border-white/20 text-white/50 hover:border-white/50'}`}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 p-1.5 space-y-1 overflow-y-auto">
          {party.map((char, idx) => {
            ...
            return (
              <button key={char.id} onClick={() => setSelectedCharIndex(idx)}
                className={`w-full px-1.5 py-1 text-left border rounded-sm transition-colors font-pixel text-[7px]
                  ${selectedCharIndex === idx ? 'border-white/60 bg-white/10 text-white' : 'border-transparent text-white/40 hover:text-white/70'}`}>
                <span>{cc?.icon} </span>{char.name}
      ...
      <div className="flex-1 p-3 overflow-y-auto">
        <AnimatePresence mode="wait">
          {activeTab === 'stat' && selectedChar && (
            <motion.div key="stat" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              <div className="border-2 border-white/30 p-3 rounded-sm">
                <h3 className="font-pixel text-sm text-white mb-2">- "{isSpanish ? selectedChar.nameEs : selectedChar.name}"</h3>
                <div className="grid grid-cols-2 gap-x-6 gap-y-1">
                  <p className="font-pixel text-[8px] text-white/70">· LV: <span className="text-white">{selectedChar.stats.level}</span></p>
                  <p className="font-pixel text-[8px] text-white/70">· EXP: <span className="text-white">{selectedChar.stats.exp}</span></p>
                  <p className="font-pixel text-[8px] text-white/70">· HP: <span className="text-white">{selectedChar.stats.hp}/{selectedChar.stats.maxHp}</span></p>
                  <p className="font-pixel text-[8px] text-white/70">· NEXT: <span className="text-white">{selectedChar.stats.expToNext - selectedChar.stats.exp}</span></p>
                </div>
              </div>
                ...
                {[
                  ...
                  <div key={stat.label} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 h-1.5 bg-white/10 rounded-sm overflow-hidden ml-2">
                      <div className="h-full rounded-sm transition-all" style={{ width: `${Math.min(100, (stat.value / 30) * 100)}%`, backgroundColor: stat.color }} />
                    </div>
                  </div>
                ))}
              </div>
              {charClass && (
                <div className="border-2 border-white/30 p-3 rounded-sm">
                  ...
                  <p className="font-pixel text-[7px] text-white/60">{isSpanish ? charClass.descriptionEs : charClass.description}</p>
                </div>
              )}
              <div className="border-2 border-white/30 p-2 rounded-sm flex justify-between">
                <p className="font-pixel text-[7px] text-yellow-400">GOLD: {gameState.gold || 0}</p>
            ...
            <motion.div key="item" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="border-2 border-white/30 p-3 rounded-sm">
                ...
                {inventoryItems.length === 0 ? (
                  <p className="font-pixel text-[8px] text-white/30 text-center py-4">{isSpanish ? '(vacío)' : '(empty)'}</p>
                ) : (
                  <div className="space-y-1">
                    {inventoryItems.map(item => (
                      <button key={item.id} onClick={() => setSelectedItem(selectedItem === item.id ? null : item.id)}
                        className={`w-full text-left px-2 py-1 font-pixel text-[8px] border rounded-sm transition-colors
                          ${selectedItem === item.id ? 'border-yellow-400 bg-yellow-400/10 text-white' : 'border-white/10 text-white/70 hover:border-white/30'}`}>
                ...
                {selectedItem && (() => {
                  ...
                  return (
                    <div className="mt-2 border-t border-white/20 pt-2">
                      <p className="font-pixel text-[7px] text-white/60">{isSpanish ? item.descriptionEs : item.description}</p>
                      {item.usable && (
                        <button onClick={() => onUseItem?.(item.id, party[selectedCharIndex]?.id || '')}
          ...
          {activeTab === 'equip' && (
            <motion.div key="equip" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="border-2 border-white/30 p-3 rounded-sm">
                ...
                <p className="font-pixel text-[7px] text-white/20 mt-3 text-center">{isSpanish ? '(Próximamente)' : '(Coming soon)'}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
